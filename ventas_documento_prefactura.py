...
Casos activos:
  1. PUBLICIDAD — variedad / producto sin descargo (propaganda).

Se eliminaron las heurísticas PREFACTURA (tamaño de documento) y BULK_DUPLICADO
(qty alta vs facturas sueltas): omitían notas de venta reales.
"""
...
# Documento consolidado tipo prefactura (legado; ya no omite descargo)
PREFACTURA_MIN_LINEAS = 8
PREFACTURA_MIN_UNI_DOC = 80
PREFACTURA_MIN_RECETAS = 6
PREFACTURA_MIN_MAX_LINEA = 29

# Línea bulk duplicada (legado; ya no omite descargo)
...
def stats_por_documento(ventas: list[dict]) -> dict[str, DocStats]:
    ...
    for doc, lines in por_doc.items():
        recetas = {
            _norm_receta(x.get("cod_receta"))
            for x in lines
            if _norm_receta(x.get("cod_receta"))
        }
...
def es_documento_prefactura(stats: DocStats) -> bool:
    """Heurística legada (ya no se usa para omitir descargo)."""
