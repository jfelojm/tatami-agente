...
def cmd_registrar_envio(args: argparse.Namespace) -> None:
    ...
    estado = (ciclo.get("estado") or "").strip()
    if estado in ESTADOS_CICLO_BLOQUEAN:
        _err("CICLO_WRONG_STATE", f"Ciclo en estado {estado}, no admite envíos")
    ...
    if not lines_in or not isinstance(lines_in, list):
        _err("VALIDATION_LINES_EMPTY", "lines debe ser un arreglo no vacío")

    lineas_db = _cargar_lineas_ciclo(sb, ciclo_id)
    if not lineas_db:
        _err("SNAPSHOT_NOT_READY", "No hay filas en conteo_linea; ejecute snapshot primero")

    clave_db = {(x["cod_mp_sistema"].strip(), x["cod_bodega"].strip()) for x in lineas_db}
    clave_payload: set[tuple[str, str]] = set()
    seen_dup: set[tuple[str, str]] = set()

    for i, L in enumerate(lines_in):
        cmp_ = (str(L.get("cod_mp_sistema", "")).strip(), str(L.get("cod_bodega", "")).strip())
        if not cmp_[0] or not cmp_[1]:
            _err("VALIDATION_CONTEO_REQUIRED", f"Línea {i}: cod_mp_sistema y cod_bodega obligatorios")
        if cmp_ in seen_dup:
            _err("VALIDATION_DUPLICATE_KEY", f"Duplicado en payload: {cmp_}")
        seen_dup.add(cmp_)
        clave_payload.add(cmp_)

        cf = L.get("conteo_fisico")
        if cf is None or (isinstance(cf, str) and not str(cf).strip()):
            _err("VALIDATION_CONTEO_REQUIRED", f"Línea {i}: conteo_fisico obligatorio (0 es válido)")
        try:
            float(cf)
        except (TypeError, ValueError):
            _err("VALIDATION_CONTEO_REQUIRED", f"Línea {i}: conteo_fisico debe ser numérico")

    if clave_payload != clave_db:
        missing = clave_db - clave_payload
        extra = clave_payload - clave_db
        if missing:
            _err(
                "VALIDATION_MISSING_LINE",
                "Faltan líneas del ciclo en el payload",
                {"faltan": [list(x) for x in sorted(missing)][:50]},
            )
        if extra:
            _err(
    ...
    idem = (args.idempotency_key or "").strip() or (body.get("idempotency_key") or "").strip()
    ...
    linea_by_key = {
        (x["cod_mp_sistema"].strip(), x["cod_bodega"].strip()): x for x in lineas_db
    }

    if not args.produccion:
        print("[DRY RUN] validación OK. Hash:", payload_hash)
        print(f"[DRY RUN] insertaría conteo_envio + {len(lines_in)} detalle y actualizaría conteo_linea")
        return
    ...
    if not envio:
        _err("DB_INSERT", "No se pudo insertar conteo_envio")
    envio_id = envio["id"]
    ...
    for L in lines_in:
        key = (
            str(L.get("cod_mp_sistema", "")).strip(),
        ...
        updates_linea.append((key[0], conteo_f, (L.get("notas") or "").strip() or None))
...
def cmd_contabilizar(args: argparse.Namespace) -> None:
    ...
    if est not in {"APROBADO_TOTAL", "APROBADO_PARCIAL"}:
        _err(
            "ENVIO_NOT_APPROVED",
            f"Estado {est}: apruebe líneas antes de contabilizar (APROBADO_TOTAL o APROBADO_PARCIAL).",
        )
    ...
    if not ciclo:
        _err("CICLO_NOT_FOUND", f"Ciclo huérfano {ciclo_id}")

    dets = _paginar(sb, "conteo_envio_detalle", "*", [("eq", "envio_id", envio_id)])
    tol = delta_abs_tol_conteo(args.tol)
    to_mov: list[dict] = []
    ...
    for d in dets:
        ...
        if delta > 0:
            ...
            mov = {
                "cod_mov": f"MOV-CONTEO+{envio_id[:8]}-{cod_mp}-{ts}",
                "fecha": fecha_mov,
                "tipo_mov": tipo,
                "cod_mp_sistema": cod_mp,
                "nombre_mp": nombre,
    ...
    if not args.produccion:
        ...
        print(
            f"[DRY RUN] generaría {len(to_mov)} mov_inventario; "
            f"{saltadas_umbral} líneas sin ajuste por umbral; "
            "cerradas como CONTABILIZADO sin cod_mov_ajuste"
        )
        for item in to_mov[:5]:
            print("  ", item["mov"]["cod_mov"], item["mov"]["tipo_mov"], item["mov"]["cantidad_mov"])
        if len(to_mov) > 5:
            print(f"  ... +{len(to_mov) - 5} más")
        return
    ...
    if pend:
        print(f"WARN: {len(pend)} líneas aún no cerradas (pendientes de aprobación o contabilización).")

    if args.cerrar_ciclo:
        sb.table("conteo_ciclo").update({"estado": "CONTABILIZADO"}).eq("id", ciclo_id).execute()

    print(
        f"OK: contabilizado envío {envio_id}; movimientos insertados: {len(to_mov)} "
        f"(umbral |delta|<{tol}: {saltadas_umbral} líneas sin mov)."
    )
    if args.recalcular_sheets:
        import subprocess
        ...
        if not os.path.exists(py):
            py = sys.executable
        subprocess.check_call([py, os.path.join(here, "recalcular_stock_sheets.py"), "--produccion"])
