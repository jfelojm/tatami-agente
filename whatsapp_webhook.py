...
async def whatsapp_webhook(
    ...
    Twilio manda application/x-www-form-urlencoded con campos como:
      - Body: texto del usuario
      - From: 'whatsapp:+593...'
    """
    text_in = (Body or "").strip()
    if not text_in:
        text_in = "(mensaje vacio)"
    ...
    # Opcional: limitar longitud por WhatsApp (Twilio suele tolerar >160, pero mejor acotar)
...
# whatsapp_webhook.py - Webhook FastAPI para agente Tatami via WhatsApp + Twilio Sandbox
import os, json
...
TOOLS = [
    {"name": "ventas_hoy", "description": "Ventas del dia actual: total en dolares, tickets, top 5 platos.", "input_schema": {"type": "object", "properties": {}, "required": []}},
...
No uses markdown, no uses asteriscos, no uses negritas. Solo texto plano."""

# Historial en memoria por numero de telefono
historiales = {}

def llamar_agente(mensaje: str, telefono: str) -> str:
    ...
    while True:
        ...
        if response.stop_reason == "end_turn" or not tool_calls:
            ...
            # Limitar historial a ultimos 10 turnos
...
async def webhook(Body: str = Form(...), From: str = Form(...)):
    ...
    try:
        respuesta = llamar_agente(mensaje, telefono)
    except Exception as e:
...
# Alias para Twilio WhatsApp (más fácil de recordar/configurar)
@app.post("/whatsapp")
async def whatsapp(Body: str = Form(...), From: str = Form(...)):
    return await webhook(Body=Body, From=From)

@app.get("/")
def health():
    return {"status": "ok", "agente": "Tatami Bao Bar"}
